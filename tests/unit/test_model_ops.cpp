...
int main() {
  ...
  OPENGIL_CHECK(changed.model_summary.prefab_updated);
  OPENGIL_CHECK(changed.model_summary.scene_updated == 9);
  OPENGIL_CHECK(changed.model_summary.preview_updated == 0);
  ...
  OPENGIL_CHECK(changed_model);
  OPENGIL_CHECK(changed_model->prefab_model_asset_id == 20001220);
  OPENGIL_CHECK(changed_model->scene_model_asset_ids.size() == 9);
  for (const auto value : changed_model->scene_model_asset_ids) {
    OPENGIL_CHECK(value == 20001220);
  }
  ...
  OPENGIL_CHECK(empty_model);
  OPENGIL_CHECK(empty_model->prefab_model_asset_id == opengil::EMPTY_MODEL_ASSET_ID);
  for (const auto value : empty_model->scene_model_asset_ids) {
    OPENGIL_CHECK(value == opengil::EMPTY_MODEL_ASSET_ID);
  }
